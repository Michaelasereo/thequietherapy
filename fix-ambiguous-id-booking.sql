-- =============================================
-- FIX AMBIGUOUS ID ERROR IN BOOKING FUNCTION
-- This fixes the "column reference 'id' is ambiguous" error
-- =============================================

-- Drop and recreate the function with all id references properly qualified
DROP FUNCTION IF EXISTS create_session_with_credit_deduction(
  UUID, UUID, DATE, TIME, INTEGER, VARCHAR, TEXT, TEXT
);

-- Recreate with ALL id references properly qualified with table prefixes
...
BEGIN
    ...
    -- Deduct credit atomically (using qualified column names)
    ...
    IF NOT FOUND THEN
        -- Rollback session creation (using qualified table and column name)
    ...
    -- Return the created session directly (all columns qualified with table alias)
    RETURN QUERY 
    SELECT 
        s.id,
        s.user_id,
...
COMMENT ON FUNCTION create_session_with_credit_deduction IS 'Atomically creates a session and deducts user credits with conflict prevention (fixed ambiguous id error - all column references qualified)';
