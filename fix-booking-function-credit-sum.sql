-- =============================================
-- FIX BOOKING FUNCTION: SUM CREDITS INSTEAD OF LIMIT 1
-- =============================================
-- The booking function should SUM all credits from both 'individual' and 'user' types
-- instead of using LIMIT 1 which can pick the wrong record
...
COMMENT ON FUNCTION create_session_with_credit_deduction IS 'Atomically creates a session and deducts user credits with conflict prevention - Only checks user_type = user (individual users only)';

-- Verify the function was created
SELECT '✅ Booking function updated to only check user_type = user (individual users only)' as status;
