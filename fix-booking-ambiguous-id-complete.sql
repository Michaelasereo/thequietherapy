-- =============================================
-- COMPLETE FIX FOR AMBIGUOUS COLUMN REFERENCE 'id'
-- =============================================
-- This fixes the "column reference 'id' is ambiguous" error
-- by using explicit column aliases and subqueries to avoid any ambiguity
...
BEGIN
    ...
    -- ✅ FIX: Insert without RETURNING, then query for ID separately to avoid ambiguity
    ...
    -- Get the session ID using a separate query to avoid RETURNING ambiguity
    ...
    IF NOT FOUND THEN
        -- Rollback session creation - use table alias to avoid ambiguity
        DELETE FROM sessions s WHERE s.id = v_session_id;
    ...
    -- ✅ FIX: Return the created session using table alias to avoid ambiguity
    -- Using table alias 's' makes it clear we're referring to table columns, not return columns
    RETURN QUERY 
    SELECT 
        s.id,
        s.user_id,
...
COMMENT ON FUNCTION create_session_with_credit_deduction IS 'Atomically creates a session and deducts user credits with conflict prevention - Fixed ambiguous column reference using explicit table qualification';

-- Verify the function was created
SELECT '✅ Booking function updated - fixed ambiguous column reference using explicit table qualification' as status;
