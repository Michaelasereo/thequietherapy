-- =============================================
-- FIX AMBIGUOUS COLUMN REFERENCE 'id' IN BOOKING FUNCTION
-- =============================================
-- The function has an ambiguous column reference 'id' because
-- the return type has a column named 'id' and we're also referencing
-- table columns named 'id'. We need to fully qualify all references.
...
BEGIN
    ...
    -- ✅ FIX: Use explicit table qualification to avoid ambiguity with return column 'id'
    RETURN QUERY 
    SELECT 
        sessions.id,
        sessions.user_id,
...
COMMENT ON FUNCTION create_session_with_credit_deduction IS 'Atomically creates a session and deducts user credits with conflict prevention - Only checks user_type = user (individual users only). Fixed ambiguous column reference.';

-- Verify the function was created
SELECT '✅ Booking function updated - fixed ambiguous column reference' as status;
