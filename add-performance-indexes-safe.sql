-- Performance Optimization: Add Missing Database Indexes (Safe Version)
-- This version uses standard PostgreSQL syntax without advanced features
-- Run these indexes one by one to ensure compatibility
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_therapist_status_time 
ON sessions (therapist_id, status, start_time DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_user_status_time 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_therapist_time_range 
ON sessions (therapist_id, start_time, end_time) 
WHERE status IN ('scheduled', 'confirmed', 'in_progress');

-- User session history
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_user_created_at 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_therapist_availability_schedule 
ON therapist_availability (therapist_id, day_of_week, start_time, end_time) 
WHERE is_available = true;

-- Day-specific availability
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_therapist_availability_day 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_therapist_profiles_verification 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_magic_links_token_auth_type 
ON magic_links (token, auth_type) 
WHERE used_at IS NULL AND expires_at > NOW();

-- Cleanup expired/used magic links
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_magic_links_expires_used 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_active_therapists 
ON users (id, full_name, email) 
WHERE user_type = 'therapist' AND is_active = true AND is_verified = true;

-- Active individual users
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_active_individuals 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_therapist_today 
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'session_notes') THEN
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_session_notes_therapist ON session_notes (therapist_id, created_at DESC)';
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_session_notes_user ON session_notes (user_id, created_at DESC)';
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_session_notes_session ON session_notes (session_id)';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'payments') THEN
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_user_status ON payments (user_id, status, created_at DESC) WHERE status IN (''completed'', ''pending'')';
        EXECUTE 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_session ON payments (session_id, status) WHERE session_id IS NOT NULL';
    END IF;
END $$;
