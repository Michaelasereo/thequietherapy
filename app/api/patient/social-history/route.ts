...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only allow individual users to access their own social history
    ...
    console.error('Error in social history GET:', error)
    return NextResponse.json({ 
      success: false, 
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only allow individual users to update their own social history
    if (session.user_type !== 'individual' && session.role !== 'individual') {
      console.error('❌ User is not an individual user:', session.user_type)
      return NextResponse.json({ success: false, error: 'Unauthorized - Not an individual user' }, { status: 403 })
    }
    ...
    // Get the social history from request body
    const socialHistory = await request.json()

    // Upsert social history
    const { data, error } = await supabase
      .from('patient_social_history')
      .upsert({
        user_id: userId,
    ...
    if (error) {
      console.error('Error upserting social history:', error)
      return NextResponse.json({ success: false, error: error.message }, { status: 500 })
    }
    ...
    console.error('Error in social history POST:', error)
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error' 
    }, { status: 500 })
