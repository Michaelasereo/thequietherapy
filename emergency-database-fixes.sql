...
BEGIN
    ...
    ELSE
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'sessions' AND column_name = 'session_url'
        ) THEN
            ALTER TABLE sessions ADD COLUMN session_url TEXT;
        END IF;
        
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'sessions' AND column_name = 'room_name'
        ) THEN
...
-- =============================================
-- STEP 7: CREATE CRITICAL INDEXES (AFTER ALL TABLES ARE READY)
-- =============================================

-- Prevent double bookings using a unique index
DO $$
BEGIN
    -- Only create the index if the status column exists
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'sessions' AND column_name = 'status'
    ) AND NOT EXISTS (
        ...
        CREATE UNIQUE INDEX unique_therapist_session_slot 
        ON sessions (therapist_id, scheduled_date, scheduled_time) 
        WHERE status IN ('scheduled', 'confirmed', 'in_progress');
        
        RAISE NOTICE 'Created unique index to prevent double bookings';
    ELSE
        RAISE NOTICE 'Skipped double-booking index creation - status column missing or index already exists';
    END IF;
END $$;
