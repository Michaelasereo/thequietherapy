...
-- =============================================
-- This is the definitive fix for the ambiguous column error
...
BEGIN
    ...
    -- ✅ FIX: Cannot qualify RETURNING clause, so get ID separately
    ...
    -- ✅ FIX: Get session ID using separate query to avoid RETURNING ambiguity
    SELECT s.id INTO v_session_id
    FROM sessions s
    WHERE s.user_id = p_user_id
      AND s.therapist_id = p_therapist_id
      AND s.scheduled_date = p_session_date
      AND s.scheduled_time = p_session_time
    ...
    IF NOT FOUND THEN
        -- ✅ FIX: DELETE without table qualification (no ambiguity in DELETE)
    ...
    -- ✅ FIX: Enqueue notifications (optional - wrapped in exception handler)
    -- Notifications are non-critical - booking should succeed even if notifications fail
    BEGIN
        ...
        PERFORM enqueue_notification(
            ...
            EXTRACT(EPOCH FROM (v_start_time - NOW() - INTERVAL '1 hour')) / 60::INTEGER -- 1 hour before session
        );
    ...
    -- ✅ FIX: Return the created session with explicit table qualification
    -- Using table name directly (sessions.column) to avoid ambiguity
    RETURN QUERY 
    SELECT 
        sessions.id,
        sessions.user_id,
...
COMMENT ON FUNCTION create_session_with_credit_deduction IS 'Atomically creates a session and deducts user credits with conflict prevention - Fixed ambiguous column reference by avoiding RETURNING clause and using explicit table qualification';

-- Verify the function was created
SELECT '✅ Booking function updated - fixed ambiguous column reference (no RETURNING clause, explicit table qualification)' as status;
